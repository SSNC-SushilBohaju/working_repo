...
public final class ElUtils {
	...
	public static final byte[] classToByteArray(ElObject eoj) {
		return new byte[]{eoj.getClassGroupCode(), eoj.getClassCode()};
	}
	
	public static final byte[] instanceToByteArray(ElObject eoj) {
	...
	public static int getElObjectCode(byte classGroupCode, byte classCode, byte instanceCode) {
	...
	public static int getElObjectCode(short echoClassCode, byte instanceCode) {
		ByteBuffer buffer = ByteBuffer.allocate(4);
		buffer.order(ByteOrder.BIG_ENDIAN);
		buffer.put((byte)0);
