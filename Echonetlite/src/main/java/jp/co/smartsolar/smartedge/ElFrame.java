...
public final class ElFrame {
	...
	protected List<ElProperty> mPropertyList;
	


	public ElFrame(short srcEchoClassCode, byte srcEchoInstanceCode
		...
		mSrcEchoAddress = ElSocket.SELF_ADDRESS;
	...
	// This constructor may create invalid ElFrame.
	public ElFrame(String srcEchoAddress, byte[] data) {
		// receive frame
		mSrcEchoAddress = srcEchoAddress;
		mDstEchoAddress = ElSocket.SELF_ADDRESS;
		...
		for(int i = 0, j = 12; i < size; i++) {
			...
			mPropertyList.add(new ElProperty(epc, pdc, edt));
		}
	}
	
	public static ElFrame getElFrameFromStream(String srcEchoAddress, DataInputStream in) throws IOException {
		...
		ElFrame frame = new ElFrame(srcEchoAddress, b);
		return frame;
		
	}
	
	protected ElFrame(short srcEchoClassCode, byte srcEchoInstanceCode,
			String srcEchoAddress
			, short dstEchoClassCode, byte dstEchoInstanceCode
			, String dstEchoAddress, byte esv, short tid, List<ElProperty> propertyList) {
		...
		mPropertyList = new ArrayList<ElProperty>();
		for(ElProperty property : propertyList) {
	...
	public List<ElProperty> getPropertyList() {
		return mPropertyList;
	}
	
	public ElProperty[] getProperties() {
		return mPropertyList.toArray(new ElProperty[]{});
	}
	...
	public void addPropertyForResponse(ElProperty property) {
	...
	public void addProperty(ElProperty property) {
		mPropertyList.add(property);
	}
	
	public ElFrame copy() { 
		ElFrame ret = new ElFrame(mSrcEchoClassCode, mSrcEchoInstanceCode
				,mSrcEchoAddress
